/** This will be a page that will manage the clients */
import React from 'react';
import Link from 'next/link';
import ListOfClients from '@/components/clients/ListOfClients';
import AddEditClient from '@/components/clients/AddEditClient';
import CreateNewClientButton from '@/components/clients/CreateNewClientButton';

export default function ClientsPage() {
    return (
      <div className="w-full max-w-xl mx-auto py-8 flex flex-wrap gap-y-6">
          <h1 className=" w-full flex items-center gap-x-4">
              <Link href={"./profile"}>
                  <span className="inline-flex justify-center items-center w-10 h-10 bg-gray-200/70 hover:bg-gray-100 rounded-xl">
                      &lt;
                  </span>
              </Link>
              <span className="text-3xl font-bold">Clients</span>
          </h1>
          <div className="flex flex-col flex- gap-y-6 flex-wrap">
            Will add here what i need :)
            <CreateNewClientButton />
            <AddEditClient />
            <ListOfClients />
          </div>
      </div>
  );
}

